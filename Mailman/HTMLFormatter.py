...
from Mailman import DlistUtils  # to support dlists
...
class HTMLFormatter:
    ...
    def FormatOptionButton(self, option, value, user):
        ...
        name = {mm_cfg.DontReceiveOwnPosts      : 'dontreceive',
                ...
                mm_cfg.SubscribedToNewThreads   : 'subnew', #for DLists
                }[option]
        return '<input type=radio name="%s" value="%d"%s>' % (
            name, value, checked)
 
    def FormatOptionSelect(self,listname):
        return '<select name="%s">' % listname 
    def FormatOptionList(self):
        ...
        while items != 0:
           ...
           return out_str
           items = items - 1
    def FormatOptionEnd(self):
        return '</select>'
    
    def FormatLists(self):
        conn = pgdb.connect(host='localhost', database='mailman_members', user='mailman', password='mailman')
        cursor = conn.cursor()    
        command = cursor.execute("SELECT listname FROM mailman_test GROUP BY listname HAVING ( count(listname) = 1  OR COUNT(LISTNAME) > 1 );")
        data = cursor.fetchall()
    ...
    def FormatOpenIDForUser(self, lang):
        return self.OpenIDOption(lang).Format()
...
#        realname = self.real_name
    ...
    def FormatFormStarts(self):
        base_url = ('http://%s:%s' % 
                    (mm_cfg.DEFAULT_URL_HOST, mm_cfg.DEFAULT_OID_PROVIDER))
 #       if extra:
        full_url = "%s" % (base_url)
#        else:
#            full_url = base_url
        return ('<FORM Method=GET ACTION="%s">' % full_url)
