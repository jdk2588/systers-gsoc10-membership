...
from Mailman.DlistUserDesc import DlistUserDesc # to support subscribe essays
...
def process_form(mlist, doc, cgidata, lang):
    ...
    # Added by Ellen to support mailing lists that require essays to join
    essay = cgidata.getvalue('essay', 'None')

    ## Begin changes by Ellen to enhance mischief checking
    mischief = None
    ...
    if not mischief:
        ...
        if len(email_split) == 2:
            local = email_split[0].lower()
            domain = email_split[1].lower()
            if local.find(mlist.internal_name()) == 0 and domain.lower() == mlist.host_name:
                mischief = 1
	...
	error=1
    ## End of Ellen's enhanced mischief checking
    #
...
email which contains further instructions.""")
    ...
    try:
        userdesc = DlistUserDesc(email, fullname, password, digest, lang, essay)
...
You have been successfully subscribed to the %(realname)s mailing list.""")
    # Show the results
    ##  Added by Ellen 5/16, I don't know why this is needed
    if results == []:
        results = privacy_results
        syslog('error', '4: Setting results to /%s/', results)
    ## End addition
