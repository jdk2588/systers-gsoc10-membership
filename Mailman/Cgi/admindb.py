...
def main():
    ...
    try:
        ...
        if not mlist.NumRequestsPending():
            ...
            if DEBUG_MODE:
                syslog('info', "thinks there are no pending requests. ")
            doc.AddItem(' ')
...
def show_pending_subs(mlist, form):
    ...
    # The Essay is there to support subscription with essay, along with below references to "essay"
    ...
    for addr, ids in byaddrs.items():
        ...
        id = ids[0]
        # essay added
        time, addr, fullname, passwd, digest, lang, essay = mlist.GetRecord(id)
        ...
        # Added  by Systers to support essay feature. Lets the list admin see the essay the subscriber has entered.
