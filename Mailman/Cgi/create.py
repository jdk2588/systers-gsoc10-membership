...
# To support the markdown used for essay questions
...
def process_request(doc, cgidata):
    """ process creating a new list using cgidata from the formular on the web """
    ...
    # support for creating dlists, added by Systers
    dlist = cgidata.getvalue('dlist', '').strip()
    # support for requiring essay, added by Systers
    essay = cgidata.getvalue('essay', '').strip()
    questions = cgidata.getvalue('questions', '').strip()
    # support for requiring fullname, added by Systers
    fullname = cgidata.getvalue('fullname', '').strip()
    ...
    # dlists_enabled, questions, essay_enabled and fullname_required are all added by Systers
    if dlist == '1':
    ...
    if essay == '1':
        mlist.essay_enabled = True
    else:
        mlist.essay_enabled = False

    if fullname == '1':
        mlist.fullname_required = True
    else:
        mlist.fullname_required = False

    # Check that the syntax for the questions only include markdown syntax, no html.
    # possible safe_mode's for markdown are escape, remove or replace
    md = markdown.Markdown(safe_mode="escape")
    questions = md.convert(questions)
    if mlist.essay_enabled:
        for wrong_markup in questions:
            if 'HTML_REMOVED' in questions:
                request_creation(
                doc, cgidata,
                _('Please only use markdown syntax for the questions, for examples see http://en.wikipedia.org/wiki/Markdown'))
                return
...
def request_creation(doc, cgidata=dummy, errmsg=None):
    """ Form for creating a list on the web """
    ...
    # control to enable dlists
    try:
        dlist = int(cgidata.getvalue('dlist', '1'))
    except ValueError:
        dlist = 1
    ftable.AddRow([Label(_('Enable dynamic sublists (listname+new)? Read more about what dynamic sublists are <a href="http://systers.org/systers-dev/doku.php/good_to_know#a_few_definitions" title="Link to wiki">here</a> ')),
                   RadioButtonArray('dlist', (_('No'), _('Yes')),
                                    checked=dlist,
                                    values=(0, 1))])
    ftable.AddCellInfo(ftable.GetCurrentRowIndex(), 0, bgcolor=GREY)
    ftable.AddCellInfo(ftable.GetCurrentRowIndex(), 1, bgcolor=GREY)

    # enable fullname required, added by Systers 
    try:
        fullname = int(cgidata.getvalue('fullname', '1'))
    except ValueError:
        fullname = 1
    ftable.AddRow([Label(_('Require subscribers to include their names as well as their email?')),
                   RadioButtonArray('fullname', (_('No'), _('Yes')),
                                    checked=fullname,
                                    values=(0, 1))])
    ftable.AddCellInfo(ftable.GetCurrentRowIndex(), 0, bgcolor=GREY)
    ftable.AddCellInfo(ftable.GetCurrentRowIndex(), 1, bgcolor=GREY)

    #check if essays should be used
    try:
        essay = int(cgidata.getvalue('essay', '1'))
    except ValueError:
        essay = 1
    ftable.AddRow([Label(_('Require subscription essay?')),
                   RadioButtonArray('essay', (_('No'), _('Yes')),
                                    checked=essay,
                                    values=(0, 1))])
    ftable.AddCellInfo(ftable.GetCurrentRowIndex(), 0, bgcolor=GREY)
    ftable.AddCellInfo(ftable.GetCurrentRowIndex(), 1, bgcolor=GREY)

    # the text for the questions in the essay, in markdown syntax
    questions = cgidata.getvalue('questions', 'This list requires subscribers to provide a short essay on their qualifications to join this list.  To join the list, you must tell us:1) your involvement in technical computing -- one or two sentences is enough, 2) that you are a woman, and 3) that you HAVE READ and AGREE to follow the rules in the [frequently asked questions](http://www.systers.org/faq.html "frequently asked questions")  Thank you!')
    ftable.AddRow([Label(_('Text for subscription essay. The essay instructions may include the lightweight markup language <a href="http://en.wikipedia.org/wiki/Markdown" Title="Link to examples of markdown syntax">markdown</a>')),
                   TextArea('questions', questions, rows=8, cols=60).Format()])
    ftable.AddCellInfo(ftable.GetCurrentRowIndex(), 0, bgcolor=GREY)
    ftable.AddCellInfo(ftable.GetCurrentRowIndex(), 1, bgcolor=GREY)
