...
# only way to Authenticates a role (in the future, we'll Authenticates a
...
class DBSecurityManager:
    ...
    def AuthContextInfos(self, authcontext, user=None):
        ...
        if authcontext == mm_cfg.AuthUser:
            ...
            secret = self.getOIDMemberPassword(user)
    ...
    def Authenticates(self, authcontexts, response, user=None):
        ...
        for ac in authcontexts:
            if ac == mm_cfg.AuthCreator:
                ...
                key, secret = self.AuthContextInfos(ac)
                if secret is None:
                    continue
                ...
                if user is not None:
                    try:
                      if self.authenticateOIDMember(user, response):
                                 return ac
    ...
    def WebAuthenticates(self, authcontexts, response, user=None):
        ...
        try:
            for ac in authcontexts:
                ok = self.CheckCookies(ac, user)
        #        ac = self.Authenticates(authcontexts, response, user)
                if ok:
                    return True
                # Check passwords
                ac = self.Authenticates(authcontexts, response, user)
                if ac:
                    print self.MakeCookies(ac, user)
                    return True
                return False
        # Added to support dlists (with the matching try:)
        except Errors.NotAMemberError:
            return False
            

    def MakeCookies(self, authcontext, user=None):
        key, secret = self.AuthContextInfos(authcontext, user)
    ...
    def ZapCookies(self, authcontext, user=None):
    ...
    def CheckCookies(self, authcontext, user=None):
        ...
        if authcontext == mm_cfg.AuthUser:
            ...
            for user in [Utils.UnobscureEmail(u) for u in usernames]:
                ok = self.__checkones(c, authcontext, user)
                if ok:
                    return True
            return False
        else:
            return self.__checkones(c, authcontext, user)

    # Written by Ellen to eliminate need for repeatedly entering username
    def InferUserFromCookies(self):
        try:
            cookiedata = os.environ.get('HTTP_COOKIE')
            ...
            verified = []
            for user in [Utils.UnobscureEmail(u) for u in usernames]:
                if self.__checkones(c, mm_cfg.AuthUser, user):
            ...
            syslog('error', 'options: CookieError: %s', cookiedata)
            return None
 
    def __checkones(self, c, authcontext, user):
