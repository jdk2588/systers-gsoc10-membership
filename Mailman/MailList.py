...
from Mailman.UserDesc import UserDesc  #do we need both?
...
from Mailman.DBSecurityManager import DBSecurityManager
...
from Mailman.DlistMemberships import DlistMemberships
from Mailman.DlistUserDesc import DlistUserDesc
...
class MailList(HTMLFormatter, Deliverer, ListAdmin,
               Archiver, Digester, SecurityManager, DBSecurityManager, Bouncer, GatewayManager,
...

    ...
    def __init__(self, name=None, lock=1):
        ...
        self._memberadaptor = DlistMemberships(self)
...
# Added to support dlists
    def GetListStartThreadEmail(self):
         """Get the address for starting new threads on this list"""
         return '%s+new@%s' % (self.internal_name(), self.host_name)
    ...
    def GetScriptURL(self, scriptname, absolute=0):
        return Utils.ScriptURL(scriptname, self.web_page_url, absolute) + \
               '/' + self.internal_name()
    def GetScriptURLS(self, scriptname, absolute=0):
        return Utils.ScriptURL(scriptname, self.web_page_url, absolute)
    ...
    def GetOpenIDURL(self, user, obscure=0, absolute=0):
        url = self.GetScriptURL('client', absolute)
        if obscure:
            user = Utils.ObscureEmail(user)
        return '%s/%s' % (url, urllib.quote(user.lower()))
...

    ...
    def AddMember(self, userdesc, remote=None):
        ...
        userdesc is an instance with the following public attributes:
            ...
            essay -- an application essay (required by some lists, added by Systers)
        ...
        # essay added by Systers
        essay = getattr(userdesc, 'essay', 'Not Submitted')  
        if digest is None:
        ...
        userdesc.essay = essay  # extra for some lists, added by Systers
        ...
        if self.subscribe_policy == 0:
            ...
            # just like above.
            # Essay is added to subscription by Systers and used for some lists
            self.HoldSubscription(email, name, password, digest, lang, essay) 
...
#            raise Errors.MMAlreadyAMember, email
    ...
    def ApprovedChangeMemberAddress(self, oldaddr, newaddr, globally):
        ...
        # do nothing.
        #changed getMemberCPAddress(newaddr) to getMemberCPAddress(oldaddr), an old Mailman bug (see https://bugs.launchpad.net/mailman/+bug/266041)
        if self.isMember(newaddr) and (self.getMemberCPAddress(oldaddr) ==
                newaddr):
            # Removed line "if self.getMemberCPAddress(oldaddr) <> newaddr:", it made no sence considering the if above.
        ...
        for listname in Utils.list_names():
            ...
            try:
                ...
                if mlist.isMember(newaddr) and (
                        mlist.getMemberCPAddress(oldaddr) == newaddr):
                    #if mlist.getMemberCPAddress(oldaddr) <> newaddr:         # removed, see logic above
    ...
    def ProcessConfirmation(self, cookie, context=None):
        ...
        if op == Pending.SUBSCRIPTION:
            ...
            try:
                ...
                # essay is added by Systers
                essay = getattr(userdesc, 'essay', 'None') 
            except ValueError:
            ...
            if invitation:
                ...
                self.HoldSubscription(addr, fullname, password, digest, lang, essay)
                name = self.real_name
                raise Errors.MMNeedApproval, _(
        ...
        for header, cre, line in self.parse_matching_header_opt():
            for value in msg.get_all(header, []):
               try:
                   if cre.search(value):
                       return line
               # Systers: This is for a problem that Ellen doesn't understand ## Annie's not sure this is right
               except TypeError, e:
                    syslog('error', 'MailList.hasMatchingHeader: %s', e)
                    try:
                        syslog('error', 'cre = /%s/', cre)
                        syslog('error', 'value = /%s/', value)
                        syslog('error', 'line = /%s/', line)
                    except:
                        syslog('error', 'Unable to print values')
                    return 0
        return 0
