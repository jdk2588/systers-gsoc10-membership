...
class OpenIDRequestHandler(BaseHTTPRequestHandler):
    ...
    def doProcess(self):
        ...
        if info.status == consumer.FAILURE and display_identifier:
            ...
            nameend = display_identifier.split('id/')
            fmt = "You have successfully verified %s as your identity. Now you can proceed to your settings<a href = 'http://%s/mailman/oiduserpage/%s'> List Page </a> " 
            
            message = fmt % (cgi.escape(display_identifier), mm_cfg.DEFAULT_URL_HOST, nameend[1])
            sreg_resp = sreg.SRegResponse.fromSuccessResponse(info)
            pape_resp = pape.Response.fromSuccessResponse(info)
            filename = ".save.txt"
            file = open(filename, 'w')
            file.write(nameend[1])
            file.close()
    ...
    def render(self, message=None, css_class='alert', form_contents=None,
               status=200, title="Systers OpenID Consumer",
...
<html>
  ...
  <body>
    <h1>%s</h1>
    <p>
      Login here with the Systers OpenID Identifier. Your Identifier should be like <b> http://%s:%s/id/<i>yourusername</i> </b>.
    </p>
''' % (title, title, mm_cfg.DEFAULT_URL_HOST, mm_cfg.DEFAULT_OID_PROVIDER))
